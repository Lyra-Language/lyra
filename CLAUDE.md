...
- **`effects.go`** — `checker.Effect`, a bitmask generalizing the old impure/pure bool (`EffectMut`, `EffectInput`/`EffectOutput` — the split of the old `EffectIO`, kept as their `Input|Output` alias — `EffectAlloc`, plus reserved-but-undetected `EffectRand`/`EffectTime`; `EffectNone` = pure). `inferImpurity` accumulates this per function/method (set-monotonic fixpoint) instead of a bool; `InferredEffects(program)` exposes it by name. Two named-bound masks over the row: **`PurityEffects = Mut|Input|Output|Rand|Time`** (everything but Alloc — `pure` and `InferredPureFunctions` are defined against it, so `EffectAlloc` is *orthogonal*, a `pure` function may allocate) and **`DetEffects = Input|Rand|Time`** (⊆ PurityEffects, so `pure` ⟹ `det`; `det` forbids only the non-determinism sources, permitting Mut/Alloc/Output — the input-vs-output IO split is what lets `det` allow logging). `EffectAlloc` detection (`purity.go`'s `allocContext`/`buildAllocContext`): constructing a `shared`-declared struct/data/named-tuple value, *unless* lexically inside a `with`-arena block (a hard-coded discharge — Lyra has no general effect handlers). Implicit allocation (dynamic arrays/strings, escaping closures) and precise arena escape are deferred to a future layout/escape pass. `builtinEffects`: print/println→Output, read→Input, write→Input|Output, `await`→Input. User surface is the `pure`/`det`/`noalloc` ladder — see `todo.md` FP/Imperative #5.
