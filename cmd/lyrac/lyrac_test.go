...
func TestBuild_BackendError(t *testing.T) {
	// unsupported.lyra type-checks and has a valid entry point but uses `yield from`
	// over a range, which the LLVM backend cannot lower yet, so lowerAndEmit reports a
	// backend error and no .ll is written. The assertion is deliberately loose (exit
	// code + "llvm backend:" prefix) so it survives changes to the exact message — or
	// a future backend that learns to lower this, at which point it fails loudly and
	// should be repointed at a still-unsupported form. (Interpolation, then array
	// literals, then `newtype`, then passing a function as a value, then struct record
	// update all used to be that form; all five lower now.)
