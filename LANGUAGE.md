...
- **`ZonedDateTime`** — a struct `{ instant, zone }`, and the local fields are computed, never stored: the clock reading `01:30` on a fall-back morning names two instants, so a struct holding `01:30` could not say which. `to_zoned_date_time(zone)`, `now_in_zone`, `to_instant`, `offset_seconds`, `abbreviation`, `is_dst`, `to_plain_date_time`/`to_plain_date`/`to_plain_time`. `Show` is RFC 9557's `2026-09-22T14:30:00-04:00[America/New_York]` — all three parts, since any two are ambiguous. **`<=>` compares instants and `==` is structural** (instant *and* zone), which is Temporal's `compare`/`equals` split.
- Not built: **the disambiguation rules** (naming a local time and asking which instant it was — the direction where a time may not exist or may happen twice), a `reject` overflow on arithmetic,
