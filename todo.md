...
### 06/22/26 (continued)
- **Fixed: tuple-destructuring never bound names** — `let (a, b) = (1, 2)` and a destructured parameter `((a, b): (i64, i64))` both type-checked the *value*/arity but never made `a`/`b` resolvable; using either name anywhere afterward was "undefined identifier". Declarations are now bound lazily in `checkDestructuringDecl` (`bindTuplePatternNames`, typechecker.go) by `Define`-ing each name directly into the real (already block-scoped) `*symbols.Scope` once the tuple's element types are known, correctly respecting `let`/`var`/`let mut` mutability and nested/sibling-scope isolation; parameters are bound eagerly in `withParamScope` (`bindTuplePatternParam`, typechecker_functions.go) since their type is already statically annotated. Array/struct/data destructuring patterns have the same gap and are not yet fixed. Tests: `destructuring_test.go`.
