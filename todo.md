...
- **Args/returns extension** (same slice, Decision (b) — owned carry a flavor, borrowed are polymorphic) — the check now also covers call arguments and return values, gated by mode: `paramOwnsArgument`/`isOwnedReturn` (`assignable.go`). Only an **`own`** parameter adopts the argument into its own storage, so only it is checked (bare/`ref`/`mut` are borrows → polymorphic → skipped); wired in `inferLambdaCall` (`typechecker_functions.go`). A return is owned-out unless the return type carries a `ref`/`mut` modifier (a borrowed return → polymorphic); wired at all return sites in `checkLambdaBody`/`checkBlockReturn` and the trait-impl method-body path (`checkTraitImplMethodBody`). Grammar already supports `own shared Node` params and `-> mut shared Node` returns (type-modifier field is separate from the allocation modifier). **Still not covered:** the `LambdaType`-callee path (`inferLambdaCallFromType`, lambda-typed struct fields — its param "modifier" is a repurposed `AllocationModifier`, murky) and element-level flavor (array/tuple interiors) — both left top-level/skipped for now. Tests: `allocation_test.go` (4 more cases: `*OwnParam*`, `*BorrowedParam*`, `*OwnedReturn*`, `*BorrowedReturn*`).
