...
- **[DONE 07/13]** `for` loops lowering (backend) — the C-style cond/body/post/exit CFG with a back-edge (`lowerForLoop`), plus `break`/`continue` (a `loops []loopCtx` stack on the lowerer; labeled break/continue walk it), and one-armed `if` statements (needed for `if cond { break }`). Introduced the block-termination discipline the earlier lowerings didn't need: `lowerBlockStmts` stops at a sealed block and every fall-through `br` is guarded by `end.Term == nil`, since break/continue are the first constructs that terminate a block mid-stream. `lowerBlock` split into a value-optional `lowerBlockStmts` + a value-requiring wrapper + `lowerForEffect` (loop/one-armed-if bodies need no value). **Only the infinite (`for {}`) and condition-only (`for cond {}`) forms are reachable** — the init/`+=` three-clause form is blocked by two frontend typechecker gaps (see below), so `Init`/`Post` lowering exists but is unexercised. **Backend limitation (same root):** a `let`/`var` declared *inside* a loop body isn't visible there (loop var must live in an enclosing scope), because the typechecker doesn't enter the loop body's scope. Tests: `TestExec_ForLoop` (accumulator, break, continue, nested labeled break).
  - **[TODO, frontend] Unblock the three-clause `for var i = 0; i < n; i += 1` form.** Two typechecker gaps: (a) `MathAssignOpExpr` (`+=`) has no `inferExprType` case, so a `+=` in value position (a block/loop body's last statement, or the loop `Post`) hits "unknown expression type" — add a case (delegate to `checkMathAssignOp`, result `void`); (b) the init-declared loop variable isn't visible in the body because `ForLoopExpr.Body` is a value copy so `checkForLoopExpr`'s `inferBlockType(&expr.Body)` enters an unregistered scope — enter the loop's own registered scope (`RecordScope(loop, loopScope)`) around the body/condition/post checks. Fixing (b) also lets a loop body declare its own locals. Backend already handles both once they type-check (Init via `lowerVarDecl`; Post needs `MathAssignOpExpr` lowering — a load/op/store, small).
