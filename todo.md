...
### 06/22/26 (continued)
- **Fixed: destructuring never bound names — tuple, struct, and array patterns** — `let (a, b) = (1, 2)`, `let {x, y} = p`, `let [a, b] = arr`, and the same patterns as destructured parameters, all type-checked the *value* but never made the bound names resolvable; using any of them afterward was "undefined identifier". Unified into one recursive `walkDestructuredPattern` (typechecker.go): tuple positions pair element-by-element (arity-checked), struct fields pair by name against the struct type (missing-field errors, partial destructuring otherwise allowed) and recurse into a field's nested pattern — which is also how the `{x: y}` rename form works, now that it's fixed (see below), array elements pair against the uniform element type with a named `...rest` binding the whole array. Declarations bind lazily in `checkDestructuringDecl` by `Define`-ing each name into the real (block-scoped) `*symbols.Scope` once the value's type is known, correctly respecting `let`/`var`/`let mut` and nested/sibling-scope isolation; parameters bind eagerly in `withParamScope` since their type is already annotated. **Side fix:** `collectStructPatternField`'s rename branch (`{x: y}`) discarded the original field name `x`, keeping only the local name `y` — field-existence checks against a renamed field were silently checking the wrong name. Now represented the same way as `{x: pattern}` (struct's own field name + a nested `IdentifierPattern` for the rename) — golden test updated. `DataPattern` destructuring (`let Some(x) = ...`) still has the same gap. Tests: `destructuring_test.go`.
