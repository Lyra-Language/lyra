...
- **[OPEN] Hover and definition on a pattern binding's own name.** Hover shows nothing or
  a wrong type (`i64` on `wh` in `Rect(...wh)`); definition answers nothing for a rest or
  struct shorthand. Both are right from a use; `patternBindingAt` is the resolver to reuse.
