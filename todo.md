...
### 06/22/26 (continued)
- **Symbol-table-backed purity — foundation, part 2: register if-let/else names; correct scoping** (FP/Imperative #3 prereq) — `if let pat = v { Then } else { Else }` and `let pat = v else { Else }` (the two forms sharing `CollectDestructuringDeclaration`) registered none of their pattern-bound names either. Fixed with correct, distinct scoping per form: if-let pushes a scope around Then only (names visible inside Then via the parent chain, NOT in Else — reaching Else means the match failed, mirrored from Rust's if-let), registered via the same `registerDestructuredNames` helper now shared with plain `let`/`var` destructuring; let-else registers into the *enclosing* scope (so subsequent statements see the names like a plain `let`) but only after collecting the diverging `Else` block, so `Else` itself never sees them. Verified no leakage via collector-level scope tests (`TestScope_IfLetBindsNamesOnlyInThenBranch`, `TestScope_LetElseBindsNamesAfterTheStatement`). Side fix: `CollectDestructuringDeclaration` passed the `type_annotation` wrapper node straight to `ParseType` instead of unwrapping its `type` field, so any if-let/else binding with a type annotation hard-errored ("unknown type node kind") — fixed alongside since it's the exact line being touched; also added the missing `IsMut` parse. **Investigated and retracted the suspected "mut-param name quirk"**: it doesn't exist — `q: mut Box` (the actual syntax; `ref`/`mut`/`own` are *type* modifiers, not pattern modifiers) registers correctly under `q`. My prior note was based on testing invalid syntax (`mut q: Box`). **Separately discovered (not yet fixed): if-let/else statements are not type-checked at all** — `typechecker.checkNode`'s switch has no case for `IfDestructuringStmt`/`ElseDestructuringStmt`, so it never descends into their bodies; a blatant type error inside an if-let's `Then` block currently passes silently. Worth its own to-do. Still to do for the full purity refactor: migrate `purity.go`'s scope machinery (the `scopeBindings`/capture-stack reconstruction) off its AST-walk onto `Scope.Parent`/`Lookup`. Tests: `scope_test.go`; full suite green.
- **Purity checker: `await` is an impure effect** (FP/Imperative #1/#3, effect-surface) — `purity.go` now flags `await` inside a `pure` function (it suspends on external I/O, breaking determinism/referential transparency) in both the main pass (`exprVisitor`) and the impurity-inference pass (`lambdaHasObservableEffect`), so an un-annotated function that awaits is inferred impure and flagged when called from a pure one. Tests: `purity_test.go`.
- **Purity checker: collect if-let / else-destructuring / `with`-arena bindings as locals** (FP/Imperative #1, effect-surface) — `declaredMutability` (`checker/purity.go`) now also handles `IfDestructuringStmt`, `ElseDestructuringStmt` (via a new `destructuredMutability` helper) and `WithStmt` (the named arena handle), so a pure function that reassigns/mutates a name bound by `if let [a,b,c] = arr { ... }` or a `with frame = ... { frame.x = ... }` is no longer falsely flagged as escaping captured state. A genuine captured mutation inside such a branch still reports. Tests: `purity_test.go`.
- **Constant-folded division-by-zero** — `isLiteralZero` (typechecker.go) now routes the integer side through `extractIntLiteralValue`, so a constant zero divisor is caught when folded, not just as a bare `0` (`10 / (5 - 5)`, `10 % (2 * 0)`). Float divisors keep the direct literal check (the folder is int-only). Tests: `math_binary_expr_test.go`.
- **Fixed: `DataPattern` as a lambda parameter mis-parsed (grammar)** — `(Some(x): Maybe<i64>) -> ...` lost the `Some(x)` parameter pattern to the constructor-call *expression* reading (`tuple_literal`), because `data_pattern` (`tree-sitter-lyra/include/patterns/index.js`) had no explicit precedence to win that tie — tree-sitter's own conflict report pinpointed the exact ambiguity: `_tuple_name` (10) vs `data_pattern` (0, unset) at the `(` immediately after a constructor name. Added `PREC.DATA_PATTERN: 10` (matching `TUPLE_PATTERN`'s existing, already-working precedence) and declared the resulting 3-way GLR conflict (`_tuple_name`, `_primary_expr`, `data_pattern`) in `grammar.js`, superseding the narrower pre-existing 2-way one. Corpus 343/343 (added one test), Go suite green. Tuple/struct/array patterns never had this problem as parameters — only data patterns did, since they're the only pattern kind that starts with a token (`user_defined_type_name`) shared with an expression form.
- **Fixed: destructuring never bound names — tuple, struct, array, *and now data* patterns** — `let (a, b) = (1, 2)`, `let {x, y} = p`, `let [a, b] = arr`, `let Some(x) = m`, and the same patterns as destructured parameters (tuple/struct/array only — see gap below), all type-checked the *value* but never made the bound names resolvable. `walkDestructuredPattern` (typechecker.go) now also handles `DataPattern`: resolves the scrutinee to its `DataType` (extending `resolveToDataType` to unwrap a generic instantiation like `Maybe<i64>`, substituting the constructor's type-variable payload for the concrete type argument via a new `substituteGenerics` — so `Some(x)` against `Maybe<i64>` correctly gives `x: i64`, not the unsubstituted `t`), matches the named constructor, and pairs its nested pattern against the constructor's positional payload types — `Some(x)` and bare `Some x` both work, as does a multi-arg/tuple-payload constructor (`MkPair (a, b)`). **Two more side fixes found along the way:** (1) `collectStructPatternField`'s rename branch (`{x: y}`) discarded the original field name `x`, keeping only the local name `y`; now represented the same way as `{x: pattern}` (golden test updated). (2) `collectDataConstructor` only recognized a fixed enum of type-node kinds for a constructor's payload and silently dropped any other kind — including `anonymous_tuple_type`, meaning the project's own recommended positional-multi-value form (`C (i64, i64)`) had an empty payload; rewritten to pick params out by the grammar's `param` field name instead of enumerating kinds. Tests: `destructuring_test.go`. (A `DataPattern` used as a *parameter* also mis-parsed at the grammar level — see the entry above, fixed separately.)
