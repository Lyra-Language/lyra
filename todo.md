...
5. **Generalize `pure` to a coarse effect row** — once 1–4 exist, model effects as a set `{io, alloc, rand, time, mut}` where `pure` == empty set. Lets you express "deterministic but allocates" (valuable for netcode/replay even when not fully pure) instead of a single boolean. Keep immutable data as plain owned **value-semantics** types (copy/move/COW), not heap-allocated persistent structures — stays layout-friendly for `fixed<>`/packed/arena.
   - **DECIDED (07/04/26) — the surface is a named-bound ladder, NOT effect-row annotations.** The coarser-than-`pure` spelling is a small set of *named* guarantees, each a union over the internal effect row:
     ...
     - **Grammar + collection + mutual-exclusion check landed (07/04/26):** `det`/`noalloc` are reserved keywords and `optional(field(...))` modifiers on `lambda_expr` (`is_det`/`is_noalloc`) and `trait_method_implementation`, mirroring `pure`; collected onto `LambdaExpr.IsDet`/`IsNoAlloc` and `TraitMethodImpl.IsDet`/`IsNoAlloc`. `checker.CheckEffectBounds` (`checker/effect_bounds.go`, `lyra-E015 CodeConflictingEffectBounds`) errors on the one contradictory pair `pure`+`det` — noalloc is orthogonal and never flagged — wired into the LSP *before* typechecking (AST-only). Corpus: `test/corpus/expressions/effect_bounds.txt`; Go tests: `checker/effect_bounds_test.go`. **Remaining follow-ups:** (1) actual `det` *enforcement* — walk the body/callees against a `DetEffects = IO|Rand|Time` mask (today the flags are collected but `det`/`noalloc` don't yet constrain anything); (2) `noalloc` enforcement via `EffectAlloc`; (3) the ambient-vs-threaded rand/time split in `builtinEffects`.
