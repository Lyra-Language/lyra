...
- ~~**Managed values inside a *stack* aggregate**~~ — **done (deep-retain-on-copy).**
  Ownership is now *deep*: the question at an owning position is whether a value
  transitively owns anything refcounted (`ownership.OwnsManaged`), not whether it is
  itself a box. Copying a stack aggregate retains each managed value it reaches (the
  per-type **retain glue**, `retain.go`), stack-aggregate bindings are framed and
  deep-released at scope exit, and `needsDrop` delegates to `OwnsManaged` so the pass
  and the backend cannot drift apart.

  This closed two real **use-after-frees**, not just a leak — an uncounted alias gets
  freed the moment the counted owner dies:

  - `let q = p; p.name = …; q.name` — interior assignment through one copy. Fixed
    twice over: copies now hold their own +1, so `releaseOldTarget` re-enabled the
    release for an inline target rooted at an owning binding (it had been suppressed,
    and leaking, as the safe stopgap).
  - `let q = ps[0]` on a `[]Person`, then let the array die — the box's drop glue
    freed the struct's `name` while `q` still pointed at it. Not fixable by
    suppressing the glue (that would reintroduce the leak it exists to close —
    freeing a list would leak the spine); only the deep retain closes it.

  **Still leaking (safe):** interior assignment through a **borrowed root** — a
  `mut`/`ref` parameter. A by-value parameter copy shares the caller's reference, so
  releasing the overwritten value there would free something the caller still owns;
  the release is suppressed and the old value leaks. That case is already
  semantically inert (a by-value `mut` parameter's mutation never reaches the
  caller), so the real fix is by-reference `mut` parameters, not more refcounting.
