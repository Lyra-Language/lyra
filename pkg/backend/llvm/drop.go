...
//
// The walk is owned_walk.go's — shared with emitRetainValue, which is the whole point:
// this pair is the ownership invariant for aggregates, and the cases both switches were
// missing (AnonymousStructType, ParameterizedType) are ones no side-by-side reading of two
// copies would have found.
func (l *lowerer) emitDropValue(block *ir.Block, v value.Value, t types.Type) (*ir.Block, error) {
	return l.emitOwnedValue(block, v, t, dropWalk)
}
