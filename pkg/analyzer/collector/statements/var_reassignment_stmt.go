...
func CollectVarReassignmentStmt(node *sitter.Node, ctx *collector_ctx.Ctx) *ast.VarReassignmentStmt {
	...
	if identNode == nil {
		ctx.AddError(node, collector_ctx.SeverityError, "var_reassignment: missing identifier")
		return nil
	}
	valueNode := node.ChildByFieldName("value")
	if valueNode == nil {
		ctx.AddError(node, collector_ctx.SeverityError, "var_reassignment: missing value")
		return nil
	}
