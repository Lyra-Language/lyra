...
// A generic `newtype` — the parameters are collected onto the declaration, not dropped.
//
// The `<t>` used to land in an ERROR node while the declaration still collected, and the
// golden recorded a ConstrainedType with the parameters *missing* — a bug documented as
// intended output, under a test named for the feature. It passed because the golden
// helper never asked whether the source parsed: a truncated AST prints fine, and a golden
// regenerated from one bakes the truncation in. That helper checks now (08/07), which is
// what surfaced this and what makes this golden trustworthy.
