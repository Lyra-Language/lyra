...
func parseCollectAndCheckYield(t *testing.T, source string) []diag.Diagnostic {
	t.Helper()
...
func assertNoYieldErrors(t *testing.T, errs []diag.Diagnostic) {
	t.Helper()
	if len(errs) > 0 {
...
func assertYieldErrorCount(t *testing.T, errs []diag.Diagnostic, count int) {
	t.Helper()
	if len(errs) != count {
