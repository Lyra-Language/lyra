...
func parseCollectAndCheckReturn(t *testing.T, source string) []diag.Diagnostic {
	t.Helper()
...
func assertNoReturnErrors(t *testing.T, errs []diag.Diagnostic) {
	t.Helper()
	if len(errs) > 0 {
...
func assertReturnErrorCount(t *testing.T, errs []diag.Diagnostic, count int) {
	t.Helper()
	if len(errs) != count {
