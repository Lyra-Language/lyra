...
func parseCollectAndCheckAwait(t *testing.T, source string) []checker.AwaitOutsideAsyncError {
	t.Helper()
...
func assertNoAwaitErrors(t *testing.T, errs []checker.AwaitOutsideAsyncError) {
	t.Helper()
	if len(errs) > 0 {
...
func assertAwaitErrorCount(t *testing.T, errs []checker.AwaitOutsideAsyncError, count int) {
	t.Helper()
	if len(errs) != count {
