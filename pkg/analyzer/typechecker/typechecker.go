...
// has to be checked *here*: `WalkStmt` descends into the target's operand rather
// than the DerefExpr itself, so a pointer write is the one raw-pointer form that
// never reaches inferExprType's DerefExpr arm.
